...
public static class Sdl3InputTracker
{
    private static readonly HashSet<SdlKey> _currentlyPressedKeys = [];
    private static readonly HashSet<SdlKey> _newKeysThisFrame = [];
    ...
    public static bool GetKey(SdlKey key)
    {
        return _currentlyPressedKeys.Contains(key);
    }

    public static bool GetKeyDown(SdlKey key)
    {
        return _newKeysThisFrame.Contains(key);
    }

    public static bool GetMouseButton(SdlMouseButton button)
    {
        return _currentlyPressedMouseButtons.Contains(button);
    }

    public static bool GetMouseButtonDown(SdlMouseButton button)
    {
    ...
    public static void UpdateFrameInput(ISdlInputSnapshot snapshot)
    {
        ...
        {
            SdlKeyEvent ke = snapshot.KeyEvents[i];
            if (ke.Down)
            {
                KeyDown(ke.Key);
            }
    ...
    private static void MouseUp(SdlMouseButton mouseButton)
    {
        _currentlyPressedMouseButtons.Remove(mouseButton);
        _newMouseButtonsThisFrame.Remove(mouseButton);
    }

    private static void MouseDown(SdlMouseButton mouseButton)
    {
    ...
    private static void KeyUp(SdlKey key)
    {
        _currentlyPressedKeys.Remove(key);
        _newKeysThisFrame.Remove(key);
    }

    private static void KeyDown(SdlKey key)
    {
...
public class SdlSimpleInputSnapshot : ISdlInputSnapshot
{
    ...
    public IReadOnlyList<SdlMouseEvent> MouseEvents => MouseEventsList;
...
public struct SdlMouseEvent
{
    ...
    public SdlMouseEvent(SdlMouseButton button, bool down)
    {
        MouseButton = button;
        Down = down;
    }
...
public struct SdlKeyEvent
{
    public SdlKey Key { get; }
    public bool Down { get; }
    public SdlModifierKeys Modifiers { get; }
    public bool Repeat { get; }

    public SdlKeyEvent(SdlKey key, bool down, SdlModifierKeys modifiers)
    : this(key, down, modifiers, false)
    {
    }

    public SdlKeyEvent(SdlKey key, bool down, SdlModifierKeys modifiers, bool repeat)
    {
        Key = key;
        Down = down;
        Modifiers = modifiers;
